...
export class FileSelectorDialogComponent implements OnInit {
  ...
  public actualCombinedFileSize(files: FileModel[] = []): number {
    if (!files) {
      files = this.data.files;
    }
    return files
      .filter((x) => x.status === FileModelStatus.Ok)
      .map((x) => x.size)
      .reduce((sum, current) => sum + current, 0);
  }
  public get acceptableExtensionsForCheck(): string[] {
    return this.acceptableExtensions.split(',').map((x: string) => {
  ...
  public get filesInformation(): string {
    ...
    if (this.configuration.fileNumberLimit > 0) {
      fileNumberPart =
        fileNumberPart + `/${this.configuration.fileNumberLimit}`;
    }
    fileNumberPart = fileNumberPart + '. ';
    let fileSizePart = `Approved files size: ${humanFileSize(
      this.actualCombinedFileSize()
    )}`;
    if (this.configuration.maximumCombinedFileSize) {
      fileSizePart =
        fileSizePart +
        `/${humanFileSize(this.configuration.maximumCombinedFileSize)}`;
    }
    if (this.configuration.maximumFileSize > 0) {
      fileSizePart =
        fileSizePart +
        `. Single file size maximum: ${humanFileSize(
  ...
  public deleteRow(item: FileModel) {
    const index = this.data.files
      .map((x: FileModel) => x.uniqueId)
  ...
  public isInProgress(fileModel: FileModel): boolean {
    return (
      fileModel.status === FileModelStatus.Initialized &&
      fileModel.progressPercentage > 0 &&
  ...
  public get saveDisabled(): boolean {
    ...
    if (
      this.compareArrays(
        this.data.files.map((x: FileModel) => x.uniqueId),
        this.initialData.files.map((x: FileModel) => x.uniqueId)
      )
    ) {
      isDisabled = true;
    }
  ...
  public get combinedTooMuchOrTooBigError(): boolean {
    return this.data.files.some(
      (x) =>
        x.status === FileModelStatus.TooBigCombinedFileSizes ||
        x.status === FileModelStatus.TooMuchCombinedFiles
    );
  }
  public get addFileDisabled(): boolean {
    return (
      this.data.files.some((x) => !x.isValid) ||
      this.combinedTooMuchOrTooBigError ||
      (this.configuration.fileNumberLimit > 0 &&
        this.data.files.length >= this.configuration.fileNumberLimit)
    );
  }
  // TRANSLATIONS START
  ...
  private getValidationModel(files: FileModel[] = []) {
    if (!files) {
      files = this.data.files;
    }
    return new ValidationModel(
      this.acceptableExtensionsForCheck,
      this.configuration.maximumFileSize,
      ...
      files.filter(
        (x: FileModel) =>
          x.status === FileModelStatus.Ok ||
          x.status === FileModelStatus.Initialized
      ).length
    );
  }
